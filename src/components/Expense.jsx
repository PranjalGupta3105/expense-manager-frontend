      ...
      <div className="w-full">
        <div className="rounded-2xl bg-white/70 dark:bg-gray-800/60 border border-gray-200/70 dark:border-gray-700 p-6">
          <div className="flex items-center gap-3">
            ...
            <p className="text-sm text-gray-700 dark:text-gray-300">Loading expenses…</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="w-full">
        ...
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <div className="w-full max-w-md rounded-2xl bg-white dark:bg-gray-800 shadow-2xl border border-gray-200 dark:border-gray-700">
              <div className="p-6">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Delete selected expenses?</h2>
                <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                  This will delete {selectedIds.length} expense{selectedIds.length === 1 ? "" : "s"}.
                </p>
      ...
      <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        ...
        <button
          className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      <div className="md:hidden space-y-3">
        {expenses.map((expense) => {
          const repayed = expense.is_repayed === 1 || expense.is_repayed === "Yes";
          return (
            <div
              key={expense.id}
              className={[
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="text-sm text-gray-500 dark:text-gray-400">{expense.data}</p>
                        <p className="mt-1 font-semibold text-gray-900 dark:text-white truncate" title={expense.description}>
                          {expense.description}
                        </p>
                      </div>
                      <button
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-lg font-bold text-gray-900 dark:text-white">₹{formatINR(expense.amount)}</p>
                      </div>
                    ...
                    <div className="mt-3 text-xs text-gray-600 dark:text-gray-300 grid grid-cols-2 gap-2">
                      <div className="truncate">
                        <span className="text-gray-500 dark:text-gray-400">Method:</span> {expense.method}
                      </div>
                      <div className="truncate">
                        <span className="text-gray-500 dark:text-gray-400">Source:</span> {expense.source}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-700/60">
              <tr className="text-xs uppercase tracking-wide text-gray-600 dark:text-gray-300">
                ...
                {columnConfig.map((c) => (
                  <th key={c.key} className={`px-4 py-3 text-left ${showAt(c.show)}`}>
                    {c.heading}
                  </th>
                ))}
                ...
                return (
                  ...
                  >
                    ...
                    {columnConfig.map((c) => {
                      ...
                      if (c.key === "amount") {
                        return (
                          <td key={c.key} className={`px-4 py-3 font-semibold text-gray-900 dark:text-white ${showAt(c.show)}`}>
                            ₹{formatINR(v)}
                          </td>
                        );
                      }
                      if (c.key === "description") {
                        return (
                          <td key={c.key} className={`px-4 py-3 max-w-[360px] ${showAt(c.show)}`}>
                            <span className="block truncate text-gray-900 dark:text-white" title={String(v ?? "")}>
                              {v}
                            </span>
                          </td>
                        );
                      ...
                      if (c.key === "is_repayed") {
                        return (
                          <td key={c.key} className={`px-4 py-3 ${showAt(c.show)}`}>
                            <span
                              className={[
                      ...
                      return (
                        <td key={c.key} className={`px-4 py-3 text-gray-700 dark:text-gray-200 whitespace-nowrap ${showAt(c.show)}`}>
                          {v}
                        </td>
                      );
                    })}
